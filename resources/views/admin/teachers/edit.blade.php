        ...
        <div class="p-6">
            <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    @if($teacher->photo)
                        <div class="mb-2">
                            <img src="{{ Storage::url($teacher->photo) }}" alt="{{ $teacher->name }}" class="h-24 w-24 rounded-full object-cover">
                        </div>
                    @endif
                ...
                <div class="mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="url" name="facebook" id="facebook" value="{{ old('facebook', $teacher->media_sosial_json['facebook'] ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label for="instagram" class="block text-xs text-gray-600">Instagram</label>
                            <input type="text" name="instagram" id="instagram" value="{{ old('instagram', $teacher->media_sosial_json['instagram'] ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label for="twitter" class="block text-xs text-gray-600">Twitter</label>
                            <input type="text" name="twitter" id="twitter" value="{{ old('twitter', $teacher->media_sosial_json['twitter'] ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label for="linkedin" class="block text-xs text-gray-600">LinkedIn URL</label>
                            <input type="url" name="linkedin" id="linkedin" value="{{ old('linkedin', $teacher->media_sosial_json['linkedin'] ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                    </div>
                </div>
